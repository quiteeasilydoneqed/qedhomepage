...
\item[\rm{(}a\rm{)}]     当 $a_1=\cdots=a_n$ 时, 有
$$
L( a_1,\cdots,a_n)=(n-1)!\int_{S^{n-1}} a_1^{x_1}\cdots a_n^{x_n}\mathrm{d} x=a_1;
$$

\item[\rm{(}b\rm{)}] 设 $m=\min\{a_1 \cdots,a_n\}$, $M=\max\{a_1 \cdots,a_n\}$, 则有
$$
\displaylines{
...
    =(n-1)!\sum_{i=1}^n \dfrac{a_i}{n!} \\
= \dfrac{\sum\limits_{i=1}^na_i}{n}.
}
$$
