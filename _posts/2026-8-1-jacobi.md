...
其次, 每个 $T\in\operatorname{SL}_2(\mathbb{C})$ 都按照分式线性变换定义了一个线性换元, 例如 $T=\begin{pmatrix} \alpha & \beta\\ \gamma &\delta \end{pmatrix}$ 定义了换元 $(z,w)\mapsto(\alpha z+\beta w,\gamma z+\delta w)$, 我们将 $L$ 换元后得到的多项式记为 $L\circ T$, 于是容易验证 $F$ 满足性质
...
接下来验证限制定义域后的 $F$ 满足性质a. 选定一点 $(a_0,b_0,c_0,d_0,e_0)\in\mathcal{R}$, 假设 $a_0\neq0$, 设 $T=\begin{pmatrix} b_0 & \frac{1}{a_0}\\ -a_0 &0 \end{pmatrix}$, 显然 $\det T=1$, 则由它定义的换元使得 $L_0(z,w)=a_0z+b_0w$ 变成 $L_0\circ T=w$, $Q_0(z,w)=c_0z^2+d_0zw+e_0w^2$ 变成 $z^2+D_0zw+E_0w^2$, 其中 $D_0=\frac{2b_0c_0}{a_0}-d_0,E_0=\frac{c_0}{a_0^2}$. 现假设 $F$ 在 $(0,1,1,D_0,E_0)$ 处为局部单射, 由 $T$ 可逆及 $F$ 与 $T$ 可交换知 $F$ 必然在 $(a_0,b_0,c_0,d_0,e_0)$ 处也是局部单射. 于是可以只考虑 $a_0=0$ 的情况.
