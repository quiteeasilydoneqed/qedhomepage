...
<b>证明</b>: 由于 $X$ 与 $Y$ 在定理中是对称的, 则可以类似的定义映射 $\psi\colon L(Y,X^*)\to L(X,Y^*)$, 并且显然是 $\varphi$ 的逆映射. 由弱$^*$拓扑的定义可知 $L(Y,X^*)$ 中算子网收敛 $T_\alpha\to T$ 当且仅当对任意 $x\in X$ 和 $y\in Y$, 标量网收敛 $T_\alpha(x)(y)\to T(x)(y)$, 而这又等价于 $L(X,Y^*)$ 中算子网收敛 $\varphi(T_\alpha)\to \varphi(T)$, 故 $\varphi$ 是连续的, 由对称性知 $\psi$ 也同样连续, 故是同胚.
</div>
